...
if default in "mM":
    rgbLower = input("Enter the lower bound RGB in format R-G-B: ")
    rgbUpper = input("Enter the upper bound RGB in format R-G-B ")
    ...
    (rUpper, gUpper, bUpper) = [130, 130, 40]
...
while ret:
    ...
    mask = cv2.inRange(frame, np.array([bLower, gLower, rLower]), np.array([bUpper, gUpper, rUpper]))
    ...
    erode_kernel = (5,5)
    eroded_image = cv2.erode(dilated_image, erode_kernel, iterations=50)
