...
class PriorityQueue(Queue):
    ...
    def firstElement(self):
        """Returns the first (p,v) pair in the queue, without removing it."""
        if self.isEmpty():
            return None
        else:
            return self.heap[0]
    ...
    def secondElement(self):
        """Returns the second-lowest (p,v) pair in the queue without removing it"""
        if self.size < 2:
            return None
        elif self.size == 2:
            # return the value at second position
            return self.heap[1]
        else:
            # second-smallest must be one of the two children of the root, so at pos 1 or 2 in heap list
            p1 = self.heap[1][0]
            p2 = self.heap[2][0]
            if p1 <= p2:
                return self.heap[1]
...
if __name__ == "__main__":
    pq = PriorityQueue()
    print(pq.firstElement(), pq.secondElement())
    print("Adding 25 apple")
    pq.enqueue(25, "apple")
    print(pq.firstElement(), pq.secondElement())
    print("Adding 12 banana")
    pq.enqueue(12, "banana")
    print(pq.firstElement(), pq.secondElement())
    print(pq)
    print("Adding 33 kiwi")
    pq.enqueue(33, "kiwi")
    print(pq.firstElement(), pq.secondElement())
    print("Updating kiwi to 5")
    pq.update(5, "kiwi")
    print(pq.firstElement(), pq.secondElement())
    print(pq)
